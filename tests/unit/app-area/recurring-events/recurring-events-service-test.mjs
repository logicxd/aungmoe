...
/* #region Automatic Sync Tests */

describe('RecurringEventsService - Automatic Sync', () => {
    let service
    let mockNotionApi
    let mockLogger
    ...
    describe('findLatestEventPerRecurringId', () => {
        it('should find latest event per recurring ID', () => {
            const events = [
                createEventWithPage({
                    notionPageId: 'page-1',
                    recurringId: 'recurring-123',
            ...
            const latestEvents = service.findLatestEventPerRecurringId(events)

            expect(latestEvents).toHaveLength(2)
            expect(latestEvents.find(e => e.recurringId === 'recurring-123').notionPageId).toBe('page-2')
            expect(latestEvents.find(e => e.recurringId === 'recurring-456').notionPageId).toBe('page-3')
        })

        it('should handle single event per recurring ID', () => {
            const events = [
                createEventWithPage({
                    notionPageId: 'page-1',
                    recurringId: 'recurring-123',
            ...
            expect(latestEvents[0].notionPageId).toBe('page-1')
        })

        it('should skip events without recurring ID', () => {
            const events = [
                createEventWithPage({
                    notionPageId: 'page-1',
                    recurringId: null,
            ...
            expect(latestEvents).toHaveLength(1)
            expect(latestEvents[0].notionPageId).toBe('page-2')
        })

        it('should return empty array for empty input', () => {
            const latestEvents = service.findLatestEventPerRecurringId([])

            expect(latestEvents).toEqual([])
        })

        it('should correctly identify latest based on dateTimeMoment', () => {
            const events = [
                createEventWithPage({
                    notionPageId: 'page-1',
                    recurringId: 'recurring-123',
                ...
                createEventWithPage({
                    notionPageId: 'page-2',
                    recurringId: 'recurring-123',
                    dateTime: '2025-10-22T10:00:00Z' // Latest
                }),
                createEventWithPage({
                    notionPageId: 'page-3',
                    recurringId: 'recurring-123',
            ...
            expect(latestEvents).toHaveLength(1)
            expect(latestEvents[0].notionPageId).toBe('page-2')
            expect(latestEvents[0].dateTimeMoment.format('YYYY-MM-DD')).toBe('2025-10-22')
        })
    })

    describe('_validateRecurringEventForAutoSync', () => {
        it('should validate event without checking isRecurringSource', () => {
            const result = service._createProcessingResult()
            const event = createEvent({
                frequency: 'Daily',
                cadence: 1,
                lookaheadNumber: 30,
                dateTime: '2025-10-08T10:00:00Z',
                isRecurringSource: false // Not marked as source
            })

            const isValid = service._validateRecurringEventForAutoSync(event, result)

            expect(isValid).toBe(true)
            expect(result.skipped).toBe(0)
        })

        it('should fail for missing frequency', () => {
            const result = service._createProcessingResult()
            const event = createEvent({
                frequency: null,
                cadence: 1,
        ...
        it('should fail for invalid cadence', () => {
            const result = service._createProcessingResult()
            const event = createEvent({
                frequency: 'Daily',
                cadence: 0,
        ...
        it('should fail for invalid lookahead number', () => {
            const result = service._createProcessingResult()
            const event = createEvent({
                frequency: 'Daily',
                cadence: 1,
        ...
        it('should fail for weekly event without recurring days', () => {
            const result = service._createProcessingResult()
            const event = createEvent({
                frequency: 'Weekly',
                cadence: 1,
    ...
    describe('getValidationFailureReasonForAutoSync', () => {
        it('should return reason without recurring source check', () => {
            const sourcePage = createSourcePage({
                frequency: 'Daily',
                cadence: 1,
                lookaheadNumber: 30,
            ...
            // Should return 'Unknown' since all fields are valid, not 'Not marked as recurring source'
            expect(reason).toBe('Unknown')
        })

        it('should return missing frequency', () => {
            const sourcePage = createSourcePage({
                frequency: null,
                cadence: 1,
                lookaheadNumber: 30,
            ...
            expect(event.getValidationFailureReasonForAutoSync()).toBe('Missing frequency')
        })

        it('should return weekly frequency requires recurring days', () => {
            const sourcePage = createSourcePage({
                frequency: 'Weekly',
                cadence: 1,
                lookaheadNumber: 4,
            ...
            expect(event.getValidationFailureReasonForAutoSync()).toBe('Weekly frequency requires recurring days')
        })
    })

    describe('processAllSourcePagesAutomatic', () => {
        it('should process only latest events per recurring ID', async () => {
            const sourcePages = [
                createSourcePage({
                    frequency: 'Daily',
                    cadence: 1,
            ...
            // Should process 2 events (latest for recurring-123 and recurring-456)
            expect(mockLogger.log).toHaveBeenCalledWith('Found 2 unique recurring event groups')
            // Should call processSourcePage for each latest event
            expect(mockNotionApi.updatePage).toHaveBeenCalled() // markEventAsProcessed
        })

        it('should skip events without recurring ID', async () => {
            const sourcePages = [
                createSourcePage({
                    frequency: 'Daily',
                    cadence: 1,
                    ...
                    dateTime: '2025-10-08T10:00:00Z',
                    isRecurringSource: true,
                    recurringId: null // No recurring ID
                })
            ]
            ...
            expect(mockLogger.log).toHaveBeenCalledWith('Found 0 unique recurring event groups')
        })
    ...
    describe('processRecurringEventsAutomatic', () => {
        it('should process events using automatic sync logic', async () => {
            mockNotionApi.getDatabase.mockResolvedValue({
                data_sources: [{ id: 'datasource-123' }]
            })
            mockNotionApi.getDataSource.mockResolvedValue({
            ...
            const result = await service.processRecurringEventsAutomatic(null)

            expect(result.errors).toEqual([])
            expect(mockLogger.log).toHaveBeenCalledWith('Found 1 unique recurring event groups')
            expect(mockLogger.log).toHaveBeenCalledWith('Automatic recurring events processing completed.')
        })
    ...
    describe('Integration: Automatic sync with mixed checkbox states', () => {
        it('should process events regardless of recurring source checkbox', async () => {
            const sourcePages = [
                createSourcePage({
                    frequency: 'Daily',
                    cadence: 1,
    ...
    describe('Integration: Automatic sync with multiple recurring ID groups', () => {
        it('should process latest event from each recurring ID group', async () => {
            const sourcePages = [
                // Group 1: recurring-123
                createSourcePage({
                    frequency: 'Daily',
                    cadence: 1,
                    lookaheadNumber: 30,
                ...
                createSourcePage({
                    ...
                    cadence: 1,
                    lookaheadNumber: 30,
                    dateTime: '2025-10-22T10:00:00Z', // Latest for group 1
                    isRecurringSource: false,
                    recurringId: 'recurring-123',
                    name: 'Event 1 - Oct 22'
                }),
                ...
                createSourcePage({
                    ...
                    cadence: 1,
                    lookaheadNumber: 4,
                    dateTime: '2025-10-17T10:00:00Z', // Latest for group 2
                    recurringDays: ['Friday'],
                    isRecurringSource: false,
                    recurringId: 'recurring-456',
                    name: 'Event 2 - Oct 17'
            ...
            expect(mockLogger.log).toHaveBeenCalledWith('Found 2 unique recurring event groups')
            expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('Event 1 - Oct 22'))
            expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('Event 2 - Oct 17'))
        })
    })
})
